...
| macOS delivery | `dist/ISO-1.0.1-macOS.zip` — pkg (3 selectable components) + MANUAL + PARAMETER-TABLE + uninstaller; payload re-opened and checked |
| Git | local repository, commit tagged `v1.0.0`; no remote |

## Windows — built and verified by CI (run 32275905943, 2026-08-18)
...
| Installer | `ISO-1.0.1-windows.exe` (Inno Setup, 4.7 MB) — installed AND uninstalled for real on the runner, payload checked on disk |
| Delivery | `dist/ISO-1.0.1-Windows-Setup.zip` — installer .exe, raw `ISO.vst3`, TRIX-shaped `INSTALL-ISO.bat` + `UNINSTALL-ISO.bat`, READ ME, MANUAL |
...
```
dist/ISO-1.0.1-macOS.zip           12 MB   pkg (VST3 / AU / app, selectable) + READ ME + MANUAL + uninstaller
dist/ISO-1.0.1-Windows-Setup.zip   6.8 MB  INSTALL-ISO.bat + ISO.vst3 + installer .exe + UNINSTALL + READ ME + MANUAL
```
